...
(function () {
    ...
    function selectJsonFile() {
        var input = document.getElementById('finupWebImportFileV231');
        if (!input) {
            input = document.createElement('input');
            input.id = 'finupWebImportFileV231';
            input.type = 'file';
    ...
    function applyWebUpdateStatus(status, errorText) {
        ...
        var menuText = errorText ? 'Updater VPS belum aktif' : webUpdateMessage(status);
    ...
    window.checkFinUpWebUpdateStatus = function (silent) {
        ...
        return fetch(FINUP_WEB_UPDATE_API + '/status?_=' + Date.now(), {
            ...
            applyWebUpdateStatus(null, 'Layanan updater VPS belum aktif. Jalankan installer updater pada server.');
            if (!silent) toast('Layanan update otomatis belum aktif di VPS.', true);
            return null;
        });
    };
    ...
    window.openFinUpWebUpdatePage = function () {
        $('modalRoot').innerHTML = '<div class="modal-wrap"><div class="modal"><div class="modal-handle"></div><h3>Update FinUp Web</h3>'
            ...
            + '<div class="notice notice-info"><b>Keamanan</b><br>Token hanya dipakai untuk satu permintaan dan tidak disimpan di browser. Pengguna tanpa token tetap dapat memeriksa versi, tetapi tidak dapat memperbarui server.</div>'
            + '<div class="action-row"><button class="btn btn-secondary" onclick="checkFinUpWebUpdateStatus(false)">Periksa lagi</button>'
            ...
            + '<p class="tiny">Jika updater belum aktif, jalankan <b>sudo bash /var/www/finup/deploy/install-finup-updater.sh</b> sekali melalui SSH.</p>'
            + '<div class="action-row"><button class="btn btn-secondary" onclick="closeModal()">Tutup</button></div></div></div>';
        pushModalHistory();
        window.setTimeout(function () { window.checkFinUpWebUpdateStatus(true); }, 0);
    };
    ...
    window.openSecuritySettings = function () {
        var html = '<div class="modal-wrap"><div class="modal"><div class="modal-handle"></div><h3>Keamanan & Privasi</h3>'
            + '<div class="notice notice-info"><b>Keamanan browser</b><br>PIN dan auto-lock melindungi tampilan FinUp pada browser ini. Browser tidak menyediakan biometrik Android atau perlindungan screenshot aplikasi.</div>'
    ...
    window.openNotificationCenter = function () {
        ...
        $('modalRoot').innerHTML = '<div class="modal-wrap"><div class="modal"><div class="modal-handle"></div>'
            ...
            + '<div class="notice notice-info">Pengingat ditampilkan di dalam FinUp Web saat aplikasi dibuka. Tidak ada pengaturan notifikasi Android pada versi web.</div>'
    ...
    window.openNotificationSettingsPage = function () {
        $('modalRoot').innerHTML = '<div class="modal-wrap"><div class="modal"><div class="modal-handle"></div><h3>Pengingat</h3>'
            + '<div class="notice notice-info"><b>Pengingat dalam aplikasi</b><br>FinUp Web menampilkan pengingat ketika aplikasi dibuka. Pengiriman notifikasi sistem Android hanya tersedia pada APK.</div>'
    ...
    window.openSettingsPage = function () {
        $('modalRoot').innerHTML = '<div class="modal-wrap"><div class="modal"><div class="modal-handle"></div><h3>Pengaturan</h3>'
            ...
            + '<div class="list-item" onclick="openFinUpWebUpdatePage()"><div class="list-main"><div class="list-icon">🔄</div><div class="list-text"><b>Update FinUp Web</b><small id="finupWebUpdateMenuStatus">Memeriksa pembaruan dari GitHub...</small></div></div><b>›</b></div>'
    ...
    window.infoContent = function (kind) {
        ...
        if (kind === 'tutorial') {
            html = html
                .replace(/<h3>14\. Pusat Pengingat dan notifikasi<\/h3><ul>[\s\S]*?<\/ul>/, '<h3>14. Pusat Pengingat</h3><ul><li>Pusat Pengingat merangkum anggaran, target, transaksi berulang, utang, dan piutang yang perlu diperhatikan.</li><li>Atur berapa hari sebelum jatuh tempo pengingat ditampilkan di dalam FinUp Web.</li><li>Notifikasi sistem Android hanya tersedia pada APK.</li></ul>')
                .replace(/<h3>17\. Keamanan dan privasi aplikasi<\/h3><ul>[\s\S]*?<\/ul>/, '<h3>17. Keamanan dan privasi web</h3><ul><li>Aktifkan PIN enam digit untuk mengunci tampilan FinUp pada browser ini.</li><li>Atur auto-lock ketika tab ditinggalkan.</li><li>Gunakan Sembunyikan nominal ketika membuka FinUp di tempat umum.</li><li>Browser tidak menyediakan biometrik Android atau perlindungan screenshot aplikasi.</li></ul>')
                .replace('tombol kembali Android', 'tombol kembali browser')
                .replace('Aktifkan PIN atau biometrik pada perangkat yang digunakan bersama orang lain.', 'Aktifkan PIN pada browser/perangkat yang digunakan bersama orang lain.');
        }
        if (kind === 'privacy') {
            html = html
                .replace('Hash PIN, preferensi biometrik, izin Android, token perangkat, cache, dan antrean offline yang tetap disimpan lokal pada masing-masing perangkat.', 'Hash PIN browser, sesi login, cache, token perangkat, dan antrean offline yang tetap disimpan lokal pada masing-masing browser/perangkat.')
